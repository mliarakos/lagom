...
private[lagom] trait ScaladslServiceApiBridge extends LagomServiceApiBridge {
  ...
  override def newRequestHeader(
    ...
    transport.RequestHeader(method, uri, requestProtocol, acceptResponseProtocols, servicePrincipal, headers)
  ...
  override def newResponseHeader(
      code: Int,
      mp: MessageProtocol,
      headers: Map[String, immutable.Seq[(String, String)]]
  ): ResponseHeader =
  ...
  override def exceptionSerializerDeserializeHttpException(
  ...
  override def exceptionSerializerDeserializeWebSocketException(
      es: ExceptionSerializer,
      code: Int,
      mp: MessageProtocol,
      bytes: ByteString
  ...
  override def exceptionSerializerSerialize(
      es: ExceptionSerializer,
      t: Throwable,
  ...
  override def newRawExceptionMessage(
      errorCode: ErrorCode,
  ...
  override def headerFilterTransformClientRequest(hf: HeaderFilter, rh: RequestHeader): RequestHeader =
    hf.transformClientRequest(rh)
  override def headerFilterTransformClientResponse(
      hf: HeaderFilter,
      resp: ResponseHeader,
      req: RequestHeader
  ): ResponseHeader = hf.transformClientResponse(resp, req)
  override def headerFilterTransformServerRequest(hf: HeaderFilter, rh: RequestHeader): RequestHeader =
    hf.transformServerRequest(rh)
  override def headerFilterTransformServerResponse(
      hf: HeaderFilter,
      resp: ResponseHeader,
      req: RequestHeader
  ): ResponseHeader = hf.transformServerResponse(resp, req)

  override type ServiceLocator = api.ServiceLocator

  override def serviceLocatorDoWithService[T](
      serviceLocator: ServiceLocator,
      descriptor: Descriptor,
      call: Call[_, _],
      block: (URI) => Future[T]
  )(implicit ec: ExecutionContext): Future[Option[T]] =
    serviceLocator.doWithService(
      descriptor.name,
      call
    )(block)
