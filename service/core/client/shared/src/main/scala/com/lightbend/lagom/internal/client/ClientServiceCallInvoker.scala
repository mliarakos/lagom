...
private[lagom] abstract class ClientServiceCallInvoker[Request, Response](
  ...
  private def makeStrictCall(
    ...
    doMakeStrictCall(method, url, headers, body).map { response =>
      // Create the message header
      val contentTypeHeader = response.headers.get(HeaderNames.CONTENT_TYPE).map(_.mkString(", "))
      val protocol          = messageProtocolFromContentTypeHeader(contentTypeHeader)
      val headers = response.headers.map {
        case (key, values) => HeaderUtils.normalize(key) -> values.map(key -> _).to[immutable.Seq]
      }
      ...
      if (response.status >= 400 && response.status <= 599) {
        ...
        responseHeader -> negotiatedDeserializerDeserialize(negotiatedDeserializer, response.body)
      }
    }
  }
...
private[lagom] case class HttpResponse(status: Int, headers: Map[String, Seq[String]], body: ByteString)
