...
trait AsciiSet {
  ...
  def toBitSet: AsciiBitSet = {
    val elems  = (0 until 256).filter(this.getInternal)
    val bitSet = BitSet(elems: _*)
 ...
 */
private[play] final class AsciiBitSet private[utils] (bitSet: BitSet) extends AsciiSet {
  ...
  private[utils] override def getInternal(i: Int): Boolean = bitSet(i)
